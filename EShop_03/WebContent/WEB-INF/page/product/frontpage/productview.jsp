...
<link href="/css/global/header01.css" rel="stylesheet" type="text/css">
	<link href="/css/product/product.css" rel="stylesheet" type="text/css">
		<link href="/css/global/topcommend.css" rel="stylesheet"
			...
			<title>${product.name}-南雄数码网</title>
						...
						*/
						function styleEvent(styleid) {
							var productImage = document
									.getElementById('productImage_' + styleid);
							if (productImage) {
						...
						function bigImageBrowse() {
							...
							var productPrototypeImage = document
									.getElementById('productPrototypeImage_'
											+ stypeid);
							if (productPrototypeImage) {
								var path = "<html:rewrite action="/product/switch"/>?method=showimage&path="
										+ productPrototypeImage.value;
			...
			<form action="<html:rewrite action="/shopping/cart"/>" method="post"
				...
				<INPUT TYPE="hidden" NAME="productid" value="${product.id}">
					...
					<div id="browse_left">
						...
						<c:forEach items="${product.productStyles}" var="style">
						...
						<div class="right_left">
							<div id="main_image" onclick="JavaScript:bigImageBrowse()"
								style="cursor:pointer;background:url(${currentimage.image140FullPath}) center center no-repeat">
								<img src="/images/global/product_blank.gif" WIDTH="200"
									HEIGHT="240" style="cursor: pointer;"/>
							</div>
							<img src="/images/global/zoom+.gif"
								onclick="JavaScript:bigImageBrowse()" style="cursor:pointer;" />
						</div>
												...
												<c:forEach items="${product.productStyles}" var="style">
													<c:if test="${style.visible}">
														<option value="${style.id }"
															<c:if test="${style.id==currentimage.id}">selected </c:if>>${style.name}</option>
													</c:if>
												</c:forEach>
											</SELECT>
										</li> 
							...
							<ul>
								<li>市场价：<s>${product.marketprice}</s> 元 <font
									color='#ff6f02'>本站价：<b>${product.sellprice} 元</b></font> 节省：<font
									color='#ff6f02'>${product.savedPrice }</font> 元
								</li>
								<li class="right_img"><INPUT TYPE="image"
									SRC="/images/global/sale.gif"></li>
								<li class="guopiprice">[ <IMG src="/images/global/2j4.gif"
									border="0">&nbsp;<A
										href="http://www.babasport.com/cache/news/6/9.shtml"
										target="_blank">配送说明</A> ]&nbsp;&nbsp;&nbsp;&nbsp;[ <IMG
										src="/images/global/2j4.gif" border="0">&nbsp;<A
											href="http://www.babasport.com/cache/news/4/24.shtml"
					...
					<div id="browse_right">
						<div id="sy_biankuang">
							<div class="sy_xinpintuijian_font">  &nbsp&nbsp&nbsp本站尚未开张</div>
						</div>
					</div>
			</form>
